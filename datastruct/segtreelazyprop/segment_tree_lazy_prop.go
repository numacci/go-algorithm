...
/*
  Verified:
    RSQ and RAQ: https://onlinejudge.u-aizu.ac.jp/solutions/problem/DSL_2_G/review/5807978/numacci/Go
	RSQ and RUQ: https://onlinejudge.u-aizu.ac.jp/solutions/problem/DSL_2_I/review/5808076/numacci/Go
*/

type SegTreeLazyProp struct {
	Size int
	...
	Fp   FP // map Fp(m, n) := m ** m ** ... ** m (n times operation; e.g. add)
	Dat  []X
	Lazy []M
}
...
type FP func(a M, b int) M

func NewSegTreeLazyProp(n int, ex X, em M, fx FX, fm FM, fa FA, fp FP) *SegTreeLazyProp {
	sg := &SegTreeLazyProp{
		Size: 1,
		Fx:   fx,
		Fm:   fm,
...
func (sg *SegTreeLazyProp) Set(k int, x X) {
	sg.Dat[k+sg.Size-1] = x
}
func (sg *SegTreeLazyProp) Build() {
...
func (sg *SegTreeLazyProp) Eval(k, sz int) {
	if sg.Lazy[k] == sg.Em {
		return
	}
	...
	// update itself
	sg.Dat[k] = sg.Fa(sg.Dat[k], sg.Fp(sg.Lazy[k], sz))
	sg.Lazy[k] = sg.Em
}

func (sg *SegTreeLazyProp) update(a, b int, x M, k, l, r int) {
	sg.Eval(k, r-l)
	if a <= l && r <= b {
...
func (sg *SegTreeLazyProp) Update(a, b int, x M) {
	sg.update(a, b, x, 0, 0, sg.Size)
}

func (sg *SegTreeLazyProp) query(a, b, k, l, r int) X {
	sg.Eval(k, r-l)
	if r <= a || b <= l {
		return sg.Ex
	}
...
func (sg *SegTreeLazyProp) Query(a, b int) X {
